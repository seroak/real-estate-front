export const getFolderList = async ({ queryKey }: { queryKey: ["folders", string] }) => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/folder/list?user_id=${userId}`);
...
export const addEstatesToFolder = async ({ folderName, estateIds }: { folderName: string; estateIds: string[] }) => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/folder/add-estate`, {
    method: "POST",
